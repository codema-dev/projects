...
dublin_buildings_at_small_area = (
    pd.concat(
        [pd.read_csv(f"data/{local_authority}_SA_2011.csv") for local_authority in ["DCC", "DLR", "FCC", "SD"]]
    )
    ...
    .assign(
        ...
        period_built_unstandardised=lambda df: df["period_built_unstandardised"].str.lower().str.replace("2006 or later", "2006 - 2011"),
    )
)
...
small_area_boundaries = gpd.read_file("data/Census2011_Small_Areas_generalised20m")[["SMALL_AREA", "EDNAME", "geometry"]].to_crs(epsg=2157)
...
postcode_boundaries = gpd.read_file("data/dublin_postcode_boundaries.geojson", driver="GeoJSON").to_crs(epsg=2157)
...
dublin_indiv_buildings_at_small_area = expand_to_indiv_buildings(dublin_buildings_at_small_area).drop(columns="value")
...
dublin_small_areas = gpd.GeoDataFrame(
    dublin_indiv_buildings_at_small_area.merge(small_area_boundaries, how="left", left_on="sa_2011", right_on="SMALL_AREA")
...
# %%
most_small_areas_linked_to_dublin_postcodes = join.centroids_within(dublin_small_areas, postcode_boundaries)

# %%
missing_sas = dublin_small_areas.merge(small_areas_linked_to_dublin_postcodes, how="left", indicator=True).query("`_merge` == 'left_only'")
...
# %%
small_areas_linked_to_dublin_postcodes = pd.concat([most_small_areas_linked_to_dublin_postcodes, missing_sas_linked])
...
# %%
dublin_indiv_buildings_at_postcode_level["regulation_period_deap_appendix_s"] = dublin_indiv_buildings_at_postcode_level.period_built_unstandardised.map(
    {
        "before 1919": "<1978",
        "1919 - 1945": "<1978",
        "1946 - 1960": "<1978",
...
# %%
dublin_indiv_buildings_at_postcode_level.to_csv("data/dublin_building_stock_up_to_2011.csv", index=False)
